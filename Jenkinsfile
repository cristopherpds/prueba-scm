pipeline {
    ...
    steps {
        script {
            ...
            sh "copy APIManager\\target\\deployPack.fed APIManager\\target\\${deployPackFileName}"

            // Publica el archivo con el nuevo nombre en Artifactory
            jf "rt u APIManager\\target\\${deployPackFileName} test-local"
            
            // Añade la propiedad de versión
            jf "rt sp test-local/APIManager/target/${deployPackFileName} build.number=${BUILD_NUMBER}"
        }
    }
}
